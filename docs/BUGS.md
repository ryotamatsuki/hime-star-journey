...
### P6-001: 湯の星地点が衝突矩形内にあり到達不能

- 発見日: 2026-06-20
- 内容: 旧仮地点 (1492, 724) は右側建物の衝突矩形内にあり、42px幅のプレイヤーが周囲の狭い隙間を通れなかった。
...
### P6-003: 湯けむりヒント会話に「星のかけら」表現が残っていた

- 発見日: 2026-06-25
- 内容: P6最終確認のEdge CDP検証中、湯の星地点へ向かう途中で湯けむりヒント会話を表示したところ、「星のかけらみたいに、きらっと光ったよ。」という表現が残っていた。P6では「湯の星のかけら」ではなく完成した一つの湯の星を扱う方針。
- 期待される挙動: 道後温泉P6導線では「湯の星」または「湯の星の光」として表現し、破片やかけら扱いにしない。
- 対応: `src/data/dialogues.ts` の該当文を「湯の星の光みたいに、きらっと光ったよ。」へ修正した。
- 検証: `rg -n "星のかけら|湯の星のかけら" src docs README.md` を実行し、`src` 側の該当がなくなったことを確認。残るヒットは今回の監査記録とDECISIONSの方針記録で、runtime会話ではない。
- 状態: 解決。
...
- 解決方法: Python 3.12.10、pip 26.1.2、Pillow 12.2.0を整備。実処理は再現可能なNode.js + Sharpの `scripts/process-chroma-assets.mjs` へ切り替え、閉領域クロマ除去、ソフトマット、despillを実施した。
- 検証: 13件すべてRGBA、透明画素・不透明被写体・四隅alpha 0を確認。暗色背景合成で大面積クロマ残りがないことを目視確認。
- 状態: 2026-06-20 解決。
