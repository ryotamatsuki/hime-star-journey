...
### P7H-000: 道後D0の歩行領域と背景の石畳がずれる

- 発見日: 2026-08-25
- 内容: 大きな矩形 `walkableRects` / `collisionRects` が、生成背景の斜め・曲線の石畳や建物・水面の輪郭と一致していなかった。
- 影響: 実画面で地面に見える場所と歩ける場所がずれ、草地・水面・屋根上へ歩けるように見える箇所があった。
- 対応: D0のwalkableRectsを背景に沿ったwalkablePolygonsへ置換し、Dogoのプレイヤー移動をコライダー四隅＋中心のポリゴン内判定へ変更。開始位置と配置点も石畳へ補正した。
- 再発防止: `maps:validate` で重要配置点・道しるべ・開始コライダー・到達性を確認し、P7 browser verifierでD0実探索の前後移動を回帰検証する。
- 状態: 解決。P10では石畳端の操作感と全手動通しを最終確認する。
