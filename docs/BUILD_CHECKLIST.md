...
- [x] Edge CDPで実ブラウザ上のパン操作、検証、保存、プレビューiframe起動を確認。
...
- [x] P6必須背景・透過レイヤー・エフェクトをAssetLoaderで読み込める。
- [x] 透過PNG 13件がRGBAで、四隅alpha 0、被写体残存を満たす。
- [x] 6シーンのプロローグとスキップが道後Exploreへ遷移する。
- [x] 湯の星取得前は星地図ボタンを隠し、Mキー遷移を拒否する。
- [x] 必須敵ID D-E01～D-E04をデータ側で定義し、撃破保存を利用する。
- [x] 条件未達では湯の星を取得できない。
- [x] 湯の星取得イベント、星地図解放、松山城解放を一度だけ保存する。
- [x] SaveData v0.2.0で不足フィールドを補完し、HP・MP・カード・撃破済み敵を保持する。
- [x] `collectedStars: ["dogo"]` の旧セーブから、星地図解放、松山城解放、道後クエストクリアIDを補完する。
- [x] typecheck / lint / build / dev HTTP 200。
- [x] Edge CDPでタイトル、6シーン、道後直接遷移、事前ロック、星地図解放、旧セーブ移行を確認。
- [x] 2026-06-25のEdge CDP補助確認でタイトル、P6代表画像HTTP 200、プロローグ6シーン、スキップ、取得前星地図ロック、コンソール/HTTP 4xxなしを確認。
- [x] 2026-06-25のEdge CDP補助確認で、必須敵撃破済みセーブから実際に湯の星地点へ歩き、湯の星取得、星地図遷移、松山城解放保存を確認。
