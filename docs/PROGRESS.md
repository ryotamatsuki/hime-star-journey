...
- 開始時は `main`、差分なし。既存作業ツリーでP6背景6枚、クロマキー人物・小物8枚、湯の星・松山城エフェクト5枚を発見し、`p6-resume-dogo-prologue` を作成した。
- Python 3.12.10、pip 26.1.2、Pillow 12.2.0とユーザーPATHを整備。P6透過処理はNode.js + Sharpの再現可能なスクリプトで実施した。
- クロマ原本13件を保持したままRGBA PNGへ処理。外周と閉領域のクロマ除去、ソフトマット、despillを適用し、四隅alpha 0、透明／不透明画素、被写体残存を検証した。
- 生成履歴が不明なP6画像について再構成プロンプトを `docs/asset-prompts/runtime-assets/p6-prologue-and-stars.prompt.md` に保存した。再生成画像はない。
- P6画像19件をAssetManifestへrequired登録した。
- PrologueScreenを6シーンのCanvas合成＋DOM会話へ置換。フェード、ズーム、パララックス、ペンダント明滅、シロ浮遊／2コマ羽ばたき、湯けむり、核吸引、異変色、影移動を実装した。
- 新規導線をTitle→Prologue→Dogo Exploreへ変更し、スキップでも同じ必須フラグを保存する。
- クエスト「湯けむりにまよった星」を追加。必須敵D-E01～D-E04、NPC／調べるヒント、目的表示、湯の星条件判定を実装した。
- 湯の星取得中は入力・接触・会話・M遷移を停止し、burst→glow→particles→完成した湯の星→白フェードを一度だけ再生する。
- 取得後にdogo星、道後クリア、星Lv2、星地図、castle、location_castle_unlockedを保存し、StarMapで道後クリアと松山城解放光を表示する。
- SaveData v0.2.0の読み込み正規化を追加。旧セーブのHP 17、MP 3、カード、D-E01撃破を保持しつつP6状態を補完するEdge CDP確認に成功した。
...
- 状態: 完了。
- 旧企画書 v0.7 完全版から、家族旅行、おばあちゃん、星守りの家系、白鷺のお守り、みかん星のペンダント、みかん星の核、シロ、カゲマサ、くろぼしのストーリー正本を復元。
- ひめの動機を「ペンダントの光を取り返す」「シロを助ける」「道後温泉と松山城を元に戻す」「星地図を完成させる」の4つとして整理。
- カゲマサの手下が奪うのはペンダント全体ではなく中央の「みかん星の核」で、外枠と弱い星地図機能は残る設定へ補正。
- `docs/specs/企画書.md` を、P6以降で参照するストーリー正本として拡張。
- `docs/specs/MVP詳細GDD.md` に、PrologueScreen、道後温泉導入、道後温泉クエスト、松山城解放、カゲマサ戦、MVP終了演出、プレイテスト観察項目の整合を追加。
- `docs/specs/MVP要件定義書.md` に、プロローグ、ペンダント受領、シロ遭遇、みかん星の核奪取、道後温泉異変、松山城解放、P6/P7以降の実装範囲を追加。
- `docs/specs/MVP実装仕様書.md` に、P6実装時に参照するストーリー実装前提を追加。
- `docs/ROADMAP.md` をP5.9、P6、P7、P8、P9、P10の新フェーズ構成へ更新。
- `docs/DECISIONS.md` に、旧企画書 v0.7 のストーリーを正本として復元する判断と、旧Phaser記述を採用しない判断を記録。
- `docs/TASKS.md` にP5.9チェックリストを追加。
- 今回はドキュメント整理のみ。ソースコード実装、画像生成、OpeningScreen実装、SaveData変更は行っていない。
