...
現在は **P6: 画像生成＋アニメーション付きプロローグ／道後温泉体験版完成** を実装済みです。全手動プレイ確認を残しつつ、次の実装フェーズは **P7: 松山城探索・松山城クエスト** です。
...
- P6で生成済み画像13件を透過処理し、画像付き6シーンプロローグ、道後温泉クエスト、湯の星取得、星地図解放、松山城解放、SaveData v0.2.0移行を実装済み。
